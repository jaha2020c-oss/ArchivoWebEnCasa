...
/* --- RUTAS DE NAVEGACIÓN (Corregidas para producción en Render) --- */
...
app.get('/canasta', (req, res) => res.sendFile(path.join(__dirname, 'public', 'canasta.html')));
...
// 2. Buscar por nombre
app.get('/api/platillos/buscar', async (req, res) => {
    try {
...
// Guardar venta con hora exacta (minutos y segundos incluidos)
app.post('/api/guardar-venta', async (req, res) => {
    ...
    try {
        ...
        const resultVenta = await pool.request()
            ...
            .input('fecha_venta', sql.DateTime, fechaHoraActual)
...
// Obtener todas las ventas para el listado del historial
