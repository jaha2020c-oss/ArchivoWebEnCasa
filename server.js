...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT id_usuario, nombre_usuario, contrasena, nombre_completo FROM Usuario WHERE nombre_usuario = @u');

        if (result.recordset.length > 0) {
            const user = result.recordset[0];
            
            // Verificación: Comparamos lo que envías con lo que hay en Azure
...
/* --- RUTAS DE NAVEGACIÓN FORZADAS --- */
// Esto asegura que si escribes /ventas.html en el navegador, el servidor lo encuentre
