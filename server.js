...
app.post('/guardar-venta', async (req, res) => {
    ...
    try {
        ...
        // 1. Obtener la hora actual del sistema (en el servidor de Render)
        const ahora = new Date();
        // 2. Calcular el desfase exacto de Nicaragua (UTC-6) en milisegundos
        const diferenciaNicaragua = -6 * 60 * 60 * 1000; 
        // 3. Crear un objeto Date limpio que refleje exactamente el reloj de Nicaragua
        const fechaActual = new Date(ahora.getTime() + (ahora.getTimezoneOffset() * 60000) + diferenciaNicaragua);

        // Registro de la Venta Principal pasando la fecha exacta con horas y segundos
        const resultVenta = await pool.request()
            ...
            .input('fecha_venta', sql.DateTime, fechaActual) // Se envía como DATETIME completo
            .execute('sp_RegistrarVenta');

        const id_venta = resultVenta.recordset[0].id_venta;
        
        // Registro secuencial del detalle de productos
...
// MODIFICADO: Ahora formatea la fecha directo a texto ISO evitando alteraciones de zona horaria
...
/* --- NUEVA API: OBTENER DETALLES DE UNA VENTA ESPECÍFICA (RECIBO) --- */
