...
/* --- 2. API: ENDPOINTS DEL DASHBOARD CON FILTROS (REVISADOS) --- */
...
/* --- RESTAURADO: HISTORIAL DE COMPRAS DEL CLIENTE frontend --- */
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT
                    ...
                    v.total_venta,
                    v.metodo_pago,
                    STRING_AGG(p.nombre_platillo + ' x' + CAST(dv.cantidad AS VARCHAR), '<br>') AS productos
                FROM Venta v
                INNER JOIN DetalleVenta dv ON v.id_venta = dv.id_venta
                INNER JOIN Platillo p ON dv.id_platillo = p.id_platillo
                WHERE v.nombre_cliente = @nombre
                GROUP BY v.id_venta, v.fecha_venta, v.total_venta, v.metodo_pago
